...
namespace Opsive.UltimateCharacterController.Audio
{
    ...
    public class AudioManager : MonoBehaviour
    {
        ...
        /// </summary>
        /// <param name="reservedGameObject">The GameObject to register.</param>
        /// <param name="volume">The volume of the new audio source.</param>
        protected virtual void RegisterInternal(GameObject reservedGameObject, float volume)
        {
            // The same GameObject can act as an AudioSource for multiple objects so it may have already been registered.
            if (m_GameObjectAudioSourcesMap.ContainsKey(reservedGameObject)) {
                return;
            }

            var audioSourcesIndex = new AudioSourcesIndex(reservedGameObject, reservedGameObject == m_GameObject, volume);
            m_GameObjectAudioSourcesMap.Add(reservedGameObject, audioSourcesIndex);
        }
        ...
        /// </summary>
        /// <param name="reservedGameObject">The GameObject that is reserving the components.</param>
        /// <param name="count">The number of components to reserve.</param>
        public void SetReserveCountInternal(GameObject reservedGameObject, int count)
        {
            AudioSourcesIndex audioSourcesIndex;
            if (!m_GameObjectAudioSourcesMap.TryGetValue(reservedGameObject, out audioSourcesIndex)) {
                Debug.LogError("Error: The GameObject " + reservedGameObject.name + " has not been registered with the AudioManager.");
                return;
            }
        ...
        /// </summary>
        /// <param name="reservedGameObject">The GameObject that is playing the audio clip.</param>
        /// <param name="clip">The clip to play.</param>
        /// <param name="pitch">The pitch to play the clip at.</param>
        /// <param name="loop">Does the clip loop?</param>
        /// <param name="delay">The number of seconds to delay the clip from playing.</param>
        /// <param name="reservedIndex">The index of the component that should be played. -1 indicates any component.</param>
        /// <returns>The AudioSource that is playing the AudioClip (can be null).</returns>
        protected virtual AudioSource PlayInternal(GameObject reservedGameObject, AudioClip clip, float pitch, bool loop, float delay, int reservedIndex)
        {
            if (clip == null) {
                return null;
            }
            ...
            AudioSourcesIndex audioSourcesIndex;
            AudioSource audioSource = null;
            if (reservedGameObject != null && reservedGameObject.activeInHierarchy) {
                if (!m_GameObjectAudioSourcesMap.TryGetValue(reservedGameObject, out audioSourcesIndex)) {
                    RegisterInternal(reservedGameObject, 1);
                    audioSourcesIndex = m_GameObjectAudioSourcesMap[reservedGameObject];
                }
        ...
        /// </summary>
        /// <param name="reservedGameObject">The GameObject to stop the audio on.</param>
        /// <param name="reservedIndex">The index of the component that should be stopped. -1 indicates all components.</param>
        private void StopInternal(GameObject reservedGameObject, int reservedIndex)
        {
            AudioSourcesIndex audioSourcesIndex;
            if (!m_GameObjectAudioSourcesMap.TryGetValue(reservedGameObject, out audioSourcesIndex)) {
                return;

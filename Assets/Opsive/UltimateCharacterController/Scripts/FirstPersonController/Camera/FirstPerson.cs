...
namespace Opsive.UltimateCharacterController.FirstPersonController.Camera.ViewTypes
{
    ...
    public abstract class FirstPerson : ViewType
    {
        ...
        public override Quaternion Rotate(float horizontalMovement, float verticalMovement, bool immediateUpdate)
        {
            ...
            if (m_CharacterLocomotion.Platform != null){
                var platformRotation = m_CharacterLocomotion.Platform.rotation;
                m_PlatformRotation = MathUtility.InverseTransformQuaternion(platformRotation, m_CharacterPlatformRotationOffset) *
                    Quaternion.Inverse(MathUtility.InverseTransformQuaternion(platformRotation, m_CharacterRotation *
                    Quaternion.Inverse(platformRotation)));
                if (!m_CharacterLocomotion.AlignToGravity) {
                    // Only the local y rotation should affect the character's rotation.
                    var characterRotation = m_CharacterTransform.rotation;
                    var localPlatformTorque = MathUtility.InverseTransformQuaternion(characterRotation, m_PlatformRotation).eulerAngles;
                    localPlatformTorque.x = localPlatformTorque.z = 0;
                    m_PlatformRotation = MathUtility.TransformQuaternion(characterRotation, Quaternion.Euler(localPlatformTorque));
                }
            ...
            if (m_CharacterLocomotion.AlignToGravity){
                var characterRotation = m_CharacterTransform.rotation;
                var localRotation = MathUtility.InverseTransformQuaternion(characterRotation, m_CharacterRotation).eulerAngles;
                localRotation.x = localRotation.z = 0;
                m_CharacterRotation = MathUtility.TransformQuaternion(characterRotation, Quaternion.Euler(localRotation));
            }
        ...
        {
            var localVelocity = m_CharacterLocomotion.TimeScale * Time.timeScale * m_Transform.InverseTransformDirection(m_CharacterLocomotion.LocomotionVelocity * 0.016f);
